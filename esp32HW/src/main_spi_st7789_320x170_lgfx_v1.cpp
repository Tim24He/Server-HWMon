...
static constexpr const char* FIRMWARE_VERSION = "spi_st7789_320x170_lgfx_v2";
static constexpr int HISTORY_CAPACITY = 480;
...
static constexpr int HISTORY_PUSH_EVERY_N_FRAMES = 2;
...
static constexpr uint16_t COLOR_TEMP = 0x6B8D;
static constexpr uint16_t COLOR_FAN = 0x9CD3;
static constexpr uint16_t COLOR_CPU = 0x5D9B;
...
void draw_graph_single(lgfx::LGFX_Sprite& dst, int x, int y, int w, int h, const MetricHistory& history,
  ...
  int py_prev_prev = py_prev_raw;
  int py_prev_prev_prev = py_prev_raw;
  ...
  for (int i = 1; i < points; ++i) {
    ...
    int py = (py_prev_prev_prev + (2 * py_prev_prev) + (2 * py_prev) + py_raw) / 6;
    ...
    dst.drawLine(px_prev, py_prev_clamped - 1, px, py - 1, color);
    py_prev_prev_prev = py_prev_prev;
    py_prev_prev = py_prev;
    px_prev = px;
    py_prev = py;
  }
}
...
void draw_graph_dual(lgfx::LGFX_Sprite& dst, int x, int y, int w, int h, const MetricHistory& a,
  ...
  draw_graph_single(dst, x, y, w, h, a, color_a, card_x, card_y, card_w, card_h, 0.0f, max_v);
  draw_graph_single(dst, x, y, w, h, b, color_b, card_x, card_y, card_w, card_h, 0.0f, max_v);
}
...
void render_dashboard() {
  ...
  canvas.drawBitmap(CARD_RIGHT_X + 5, CARD_BOT_Y + 3, image_choice_right_bits, 16, 16, COLOR_CPU);
  ...
  canvas.setTextColor(COLOR_FAN, COLOR_CARD);
  draw_centered_in_card_header_bold(canvas, CARD_LEFT_X, CARD_COL_W, CARD_MID_Y + 2, fan_text);
  ...
  draw_graph_single(canvas, graph_mid_x, graph_mid_y, graph_mid_w, graph_mid_h, fan_history, COLOR_FAN, CARD_LEFT_X, CARD_MID_Y, CARD_COL_W, CARD_MID_H);
  draw_graph_dual(canvas, graph_mid_r_x, graph_mid_r_y, graph_mid_r_w, graph_mid_r_h, up_history, down_history, COLOR_UP, COLOR_DOWN, CARD_RIGHT_X, CARD_MID_Y, CARD_COL_W, CARD_MID_H);
  draw_graph_single(canvas, graph_bot_x, graph_bot_y, graph_bot_w, graph_bot_h, temp_history, COLOR_TEMP, CARD_LEFT_X, CARD_BOT_Y, CARD_COL_W, CARD_BOT_H);
  draw_graph_single(canvas, graph_bot_r_x, graph_bot_r_y, graph_bot_r_w, graph_bot_r_h, cpu_history, COLOR_CPU, CARD_RIGHT_X, CARD_BOT_Y, CARD_COL_W, CARD_BOT_H);
...
void setup() {
  ...
  // --- FORCE CLEAN ST7789 ADDRESSING WINDOW ---
  lcd.writecommand(0x36); // MADCTL
  lcd.writedata(0x00);

  lcd.writecommand(0x2A); // CASET
  lcd.writedata(0x00);
  lcd.writedata(0x00);
  lcd.writedata(0x00);
  lcd.writedata(0xEF);

  lcd.writecommand(0x2B); // RASET
  lcd.writedata(0x00);
  lcd.writedata(0x00);
  lcd.writedata(0x01);
  lcd.writedata(0x3F);
